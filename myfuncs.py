def Potential_Coul_multip(r,Z,R0,e0,epsilon0,beta2,beta4):
    ...
    import numpy as np
    Y00=lambda theta: 1/np.sqrt(4*np.pi)+0*np.cos(theta)
    ...
    lam=0
    V_Coul_0 =Y00(0)*1/(4*np.pi*epsilon0)/1e-15/(e0*1e6)*3*Z*e0**2/(R0**3)*2*np.pi/(2*lam+1)*2*np.trapz(Klambda(lam,r,Rcoul_theta(theta_grid))*Y00(theta_grid)*np.sin(theta_grid)*dtheta_grid)

    # order 2
    lam=2
    V_Coul_2 =1/(4*np.pi*epsilon0)/1e-15/(e0*1e6)*3*Z*e0**2/(R0**3)*2*np.pi/(2*lam+1)*2*np.trapz(Klambda(lam,r,Rcoul_theta(theta_grid))*Y20(theta_grid)*np.sin(theta_grid)*dtheta_grid)
    
    # order 4
    lam=4
    V_Coul_4 =1/(4*np.pi*epsilon0)/1e-15/(e0*1e6)*3*Z*e0**2/(R0**3)*2*np.pi/(2*lam+1)*2*np.trapz(Klambda(lam,r,Rcoul_theta(theta_grid))*Y40(theta_grid)*np.sin(theta_grid)*dtheta_grid)
